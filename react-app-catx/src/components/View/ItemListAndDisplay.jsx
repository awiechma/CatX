...
const ItemListAndDisplay = ({ searchString, selectedTags }) => {
  ...
  const fetchItems = async () => {
    ...
    setError(null);
    try {
      const endpoint = `http://localhost:3000/stac/search?${
  ...
  if (isLoading)
    return <div className="custom-container bg-body-tertiary">Loading...</div>;
  if (error)
    return (
      <div className="custom-container bg-body-tertiary">Error: {error}</div>
    );
